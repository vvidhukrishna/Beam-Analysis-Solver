...
def save_analysis_history(beam, system_type, reactions, x_grid, shear_force, bending_moment, summary_statistics):
    ...
    history_entry = {
        ...
        "beam": {
            "length": beam.length,
            "supports": [{"x": x, "type": support.support_type} for x, support in beam.supports()],
            "point_loads": [{"x": x, "force": load.force} for x, load in beam.point_loads()],
            "applied_moments": [{"x": x, "moment": moment.moment}for x, moment in beam.applied_moments()],
            "distributed_loads": []},
        "analysis": {
            "system_type": system_type,
            "reactions": reactions,
            "summary_statistics": summary_statistics,
            "analysis_data": {"x_coords": x_grid.tolist(), "shear_force": shear_force.tolist(), "bending_moment": bending_moment.tolist()}}}
...
def reconstruct_beam(entry):
    ...
    for dist in beam_data.get("distributed_loads", []):
        if dist["type"] == "udl":
            udl = UniformDistributedLoad(dist["start_x"], dist["end_x"], dist["intensity"])
            beam.add_distributed_event(udl)

        elif dist["type"] == "uvl":
            uvl = UniformVaryingLoad(dist["start_x"], dist["end_x"], dist["w1"], dist["w2"])
